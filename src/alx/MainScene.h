...
public:
    ...
    void update_tick_simulation(float dt) {
        ...
        if (m_sim_timer >= SIM_TICK_RATE) {
            ...
            if (sim_res.stagnant_decay_events > 0) {
                m_twilight_level += TWILIGHT_DECREASE_PER_MANA * sim_res.stagnant_decay_events;
                m_twilight_level = std::clamp(m_twilight_level, 0.0f, TWILIGHT_MAX);
            }
        }
    }
    ...
    void draw_fixture_bg(const Fixture& fix, int gx, int gy, int screen_x, int screen_y, int tile_size) {
        ...
        if (fix.type == FixtureType::Refiner) {
            color = 0xFF301C66;
        } else if (fix.type == FixtureType::Spire || fix.type == FixtureType::Seep) {
            color = 0xFF00FF66;
        }
    ...
    void draw_pipe_mana(const Fixture& fix, int gx, int gy, int screen_x, int screen_y, int tile_size, float progress) {
        uint32_t liquid_color = (fix.mana_state == ManaState::Light) ? 0xFF00FFFF : 0xFF9900FF;
        int stream_w = 4;
        int offset = (tile_size - stream_w) / 2;

        int anim_offset_x = static_cast<int>(-fix.move_dx * (1.0f - progress) * tile_size);
        int anim_offset_y = static_cast<int>(-fix.move_dy * (1.0f - progress) * tile_size);
        ...
        if (fix.move_dx != 0 || fix.move_dy != 0) {
            if (fix.move_dx != 0) {
                ...
                if (is_connectable_fixture(gx - 1, gy) || fix.move_dx == 1) {
                ...
                if (is_connectable_fixture(gx, gy - 1) || fix.move_dy == 1) {
    ...
    void draw_fixture_powered(const Fixture& fix, int screen_x, int screen_y, int tile_size) {
        ...
        Draw::rect(screen_x, screen_y, tile_size, tile_size, color, false, 1, 0);
    }
